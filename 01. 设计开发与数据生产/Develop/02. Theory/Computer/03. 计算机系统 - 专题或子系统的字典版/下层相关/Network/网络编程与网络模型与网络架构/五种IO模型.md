...
无论是Java还是其他的语言，本质上IO读写操作的原理是类似的，编程语言开发的程序，一般都是工作在用户态空间，但由于IO读写对于计算机而言，属于高危操作，所以OS不可能100%将这些功能开放给用户态的程序使用，所以正常情况下的程序读写操作，本质上都是在调用OS内核提供的函数：read()、 write()。
也就是说，在程序中试图利用IO机制读写数据时，仅仅只是调用了内核提供的接口函数而已，本质上真正的IO操作还是由内核自己去完成的。

IO工作的过程如下：

![屏幕截图(2)](五种IO模型.assets/2a54147a7c6d63c5d1195af2b2d7d762.png)
...
![1653896065386](五种IO模型.assets/649190a2fac839869d3216ce39e96bb8.png)
...
![1653896687354](五种IO模型.assets/10da93625491179f3fcd1fb7aa2d7760.png)
...
Linux的五种IO模型浅析（从性能上来说，它们属于依次递进的关系，但越靠后的`IO`模型实现也越为复杂）
...
这也是最初的`IO`模型，也就是当调用内核的`read()`函数后，内核在执行数据准备、复制阶段的`IO`操作时，应用线程都是阻塞的，所以本次`IO`操作则被称为同步阻塞式`IO`，如下：

![1653897115346](五种IO模型.assets/6ce8236039f4d79bf79e2b956eb67b17.png)
...
![1653897270074](五种IO模型.assets/65cd478c4d23e609b7f9ec94a3d1a8db.png)
...
![image-20230401212536418](五种IO模型.assets/7b6bcc1923e8dee606855747067bacd2.png)
...
![image-20230401212851013](五种IO模型.assets/753cc2a5d2e15e9c98df03431c89677d.png)
...
![image-20230426140036779](五种IO模型.assets/361737f5aeed9da97204d746277fc717.png)

### NIO，同步非阻塞模型 (Non-Blocking-IO)

特征：多线程、异步、非阻塞

从字面意思上来说就是:调用`read()`函数的线程并不会阻塞，而是可以正常运行，如下：

![1653897490116](五种IO模型.assets/ecef38c4d8f8360c61a62a9b4ab4720a.png)
...
> 其实相对来说，这个过程虽然没有阻塞发起`IO`调用的线程，但实际上也会让调用方不断去轮询发起“数据是否准备好”的信号，这也并非真正意义上的非阻塞。
...
![image-20230401213439953](五种IO模型.assets/64a3e9750ba36eeab85ffcb7e14a1c19.png)
...
![1653898691736](五种IO模型.assets/73d7b05448aa92d3efd20c4ea3a58de2.png)
...
> [【操作系统】全面解析IO 多路复用：select、poll、epoll](https://blog.csdn.net/weixin_63566550/article/details/129670560?spm=1001.2014.3001.5501)

![img](五种IO模型.assets/9e3d4e62c9b229ab87df815f4ccf5984.png)
...
![img](五种IO模型.assets/60cd00951dedeaac47e94fa20485861f.png)
...
> 信号驱动模型相较于之前的模型而言，**从一定意义上实现了异步**，也就是数据的**准备阶段**是异步非阻塞执行的，但数据的**复制阶段**却依旧是同步阻塞执行的。
...
该模型是**真正意义上的异步非阻塞**式`IO`，代表数据准备与复制阶段都是异步非阻塞的。

他会由内核将所有数据处理完成后，由内核将数据写入到用户态中，然后才算完成，所以性能极高，不会有任何阻塞，全部都由内核完成，可以看到，异步IO模型中，用户进程在两个阶段都是非阻塞状态。

![img](五种IO模型.assets/e4ded2cc4cbf9e567f11d1124833435d.png)
...
![屏幕截图(2)](五种IO模型.assets/1fa90416b65638851c6967862a566a03.png)
...
要想客户端和服务器能在网络中通信，那必须得使用 Socket 编程，它是[进程间通信](https://so.csdn.net/so/search?q=进程间通信&spm=1001.2101.3001.7020)里比较特别的方式，特别之处在于它是可以跨主机间通信。
