...
这是两个模块层偶尔很低，Server层 通过API 调用存储引擎层，前后者不互相依赖，像存储引擎可以随意替换成不同的其他引擎
...
- 职责
  - 建立连接、分析 SQL、执行 SQL
- 功能
  - 大多数的*核心功能模块*（主要包括：连接器，查询缓存、解析器、预处理器、优化器、执行器 等）
  - 所有的*内置函数*（如：日期、时间、数学、加密函数 等）
  - 所有*跨存储引擎的功能*（如：存储过程、触发器、视图 等）
...
![查询语句执行流程](assets/mysql查询流程-1725354426217-27.png)
...
![img](assets/db-mysql-sql-parser-2.png)
...
[ ](https://xiaolincoding.com/mysql/index/index_interview.html#按字段个数分类)

[ (opens new window)](https://xiaolincoding.com/mysql/index/index_interview.html#按字段个数分类)
