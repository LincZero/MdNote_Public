...
默认的 Public 目录是 `.vuepress/public` ，可以通过 [public](https://vuepress.github.io/zh/reference/config.html#public) 配置项来修改。
...
如果你的网站部署在非根路径下，例如 `https://foo.github.io/bar/` ，那么你应该把 [base](https://vuepress.github.io/zh/reference/config.html#base) 设置为 `'/bar/'`。显然，此时你的 Public 文件会被部署在 `https://foo.github.io/bar/images/hero.png` 这样的链接下。
...
然而，有些情况下，你可能会有一些指向 Public 文件的动态路径，尤其是在你开发一个自定义主题的时候。在这种情况下， `base` 无法被自动处理。为了解决这个问题，VuePress 提供了 [withBase](https://vuepress.github.io/zh/reference/client-api.html#withbase) 工具函数，它可以帮助你添加 `base` 前缀：
...
> [!TIPS]
>
> 配置参考： [alias](https://vuepress.github.io/zh/reference/plugin-api.html#alias)
