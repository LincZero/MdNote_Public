...
- [标记装饰 ](https://codemirror.net/docs/ref/#view.Decoration^mark)添加样式或 DOM   给定范围内文本的属性。 
- [小部件装饰 ](https://codemirror.net/docs/ref/#view.Decoration^widget)插入一个 DOM 元素在文档中的给定位置。 
- [更换装饰 ](https://codemirror.net/docs/ref/#view.Decoration^replace)隐藏部分文档或用给定的 DOM 节点替换它。 
- [线条装饰 ](https://codemirror.net/docs/ref/#view.Decoration^line)可以给a添加属性行的换行元素。 
...
注意区分`Decoration.replace` `Editor.rangeReplace` `HTMLElement.replaceWith`，分别是替换装饰、替换文字、HTML元素替换
...
Plugin --> ViewPlugin: function variable
ViewPlugin --> PluginValue: function variable
PluginValue --> DecorationSet: has、间接create

PluginValue ..> RangeSetBuilder: 1. create DecorationSet
RangeSetBuilder ..> DecorationSet: 2. create DecorationSet
RangeSetBuilder ..> Decoration: 《T》、function variable
DecorationSet ..> Decoration: 《T》

class Plugin{
	registerEditorExtension: void(Extension)
}

class ViewPlugin{
	fromClass: ViewPlugin(EditorView, PluginSpec)
}
class PluginValue{
	+decorations: DecorationSet
	-builderDecoration: DecorationSet()
}
...
SelectionRange ..> StateEffectType1: as of() \n function variable
underlineSelection ..> StateEffect_List:has list

StateEffect_List ..> StateEffectType1: define by \n StateEffect.define()
StateEffectType1 ..> StateEffect1: return by of()
StateEffect_List --> StateEffect1: ① be pushed - list has, \n define by \n StateEffect.define().of()，\n 因为可以有多个选择段,\n这边其实是列表

StateEffect_List --> StateEffect2: ③ be pushed - list has, \n define by \n StateEffect.appendConfig().of()
StateEffect_List ..> StateEffectType2: define by \n StateEffect.appendConfig()
StateEffectType2 ..> StateEffect2: return by of()
StateField ..> StateEffectType2: as of() \n function variable
Extension1 ..> StateEffectType2: as of() \n function variable

N --> StateField: ② function variable, \n by EditorView.state.field()
N ..> StateFieldSpec: create by \n StateEffect.define()
StateFieldSpec ..> StateField : as define() \n function variable

Decoration ..> DecorationSet: as return
DecorationSet ..> Decoration: 《T》
DecorationSet ..> StateFieldSpec: as update() \n function variable
Transaction ..> StateFieldSpec: as update() \n function variable


class underlineSelection{
	effects: StateEffect[]
	construct(EditorView view)
}
<<function>> underlineSelection

class StateEffect1{
	《T》: unknown
	static StateEffectType appendConfig
	static StateEffectType define(StateEffectSpec)
}
<<T_unknown>> StateEffect1
class StateEffectType1{
	《T》: number number
	+StateEffect of(《T》)
}
<<T_number_number>> StateEffectType1
...
<<T_Extension>> StateEffect2
class StateEffectType2{
	《T》: Extension
	+StateEffect of(《T》)
}
<<T_Extension>> StateEffectType2

class StateField{
	《T》: DecorationSet
	state StateField《T》 define(StateFieldSpec《T》)
}
<<T_DecorationSet>> StateField

class StateFieldSpec{
	《T》: DecorationSet
	+《T》 create(EditorState)
	+《T》 update(《T》, Transaction)
}

class Transaction{
	readonly effects: StateEffect[]
}
...
（这版没有ViewPlugin和PluginValue，但感觉更难懂了。StateField有嵌套泛型，`effects: StateEffect[]`列表里的类型居然是不统一的！麻了）
...
>           也说明了为什么设置有垂直高度的块装饰时，需要用`EditorView.decorations`将装饰放在**状态字段**中，而不是**视图插件**中
...
>   装饰品是通过一个 [facet](https://codemirror.net/docs/ref/#view.EditorView^decorations)。每次景色都已更新，此方面的内容用于设置可见样式内容。 
>
>   装饰品 [成套 ](https://codemirror.net/docs/ref/#state.RangeSet)保存，又是   不可变的数据结构。   这样的集合可以是  [ 跨变化映射 ](https://codemirror.net/docs/ref/#state.RangeSet.map)（调整   其内容的位置以补偿变化）或  [重建 ](https://codemirror.net/docs/ref/#state.RangeSetBuilder) 根据使用情况 更新  案件。 
>
>   有两种方式可以提供装饰：直接，通过将范围设置值放在构面中（通常由  [ 从字段中派生 ](https://codemirror.net/docs/ref/#state.StateField^define^config.provide)），   或者间接地，通过提供从视图到范围集的功能。 
>
>   只有直接提供的装饰套可能会影响垂直   编辑器的块结构，但只有间接提供的才可以   阅读编辑器的视口（如果你愿意，这可能很有用，因为   例如，仅装饰 [可见 的内容 ](https://codemirror.net/docs/ref/#view.EditorView.visibleRanges)）。   这样做的原因限制是视口是从块计算的 结构，所以在读取视口之前必须知道它。 
>



#### [原子范围](https://codemirror.net/examples/decoration/#Atomic%20Ranges)
...
主要原因应该是**影响了编辑器的垂直块结构**，官网的Guide页面提过这个注意要项：
...
-   Decorations that replace line breaks may not be specified via plugins
    -   [【Obsidian论坛】如何创建自定义 CodeMirror 块小部件？](https://forum.obsidian.md/t/how-to-create-custom-codemirror-block-widget/36132)，这个没有回答
    -   [【CodeMirror论坛】创建多行小部件的首选方法？](https://discuss.codemirror.net/t/preferred-way-to-create-multi-line-widget/4865)，同下
-   Block decorations may not be specified via plugins
    这几个问题的回答者都是 “marijn”（CodeMirror维护者），是有参考价值的
    -   [【CodeMirror论坛】块装饰错误 - 未捕获的 RangeError：可能无法通过插件指定块装饰](https://discuss.codemirror.net/t/block-decoration-error-uncaught-rangeerror-block-decorations-may-not-be-specified-via-plugins/4102)
        说要用 `decorations facet`
    -   ~~[【CodeMirror论坛】块装饰不能通过插件指定](https://discuss.codemirror.net/t/block-decorations-may-not-be-specified-via-plugins/4812)~~
        ~~说这是不可能的~~
    -   [【CodeMirror论坛】创建多行小部件的首选方法？](https://discuss.codemirror.net/t/preferred-way-to-create-multi-line-widget/4865)
        说`EditorView.decorations`. 将装饰放在状态字段中，而不是插件中，以便它们在计算视口之前可用。
    -   [【CodeMirror论坛】应当使用facet弄一个原子范围](https://discuss.codemirror.net/t/add-class-to-gutter-of-line/4602/6)
        看[装修例子 5个](https://codemirror.net/examples/decoration/)—修改编辑器块结构的装饰必须**直接提供**，而不是作为视图的函数
...
>   使用数据结构向编辑器提供装饰，该 [`RangeSet`](https://codemirror.net/docs/ref/#state.RangeSet)数据结构存储值的集合（在本例中为装饰）以及与之关联的范围（开始和结束位置）。这种数据结构有助于在文档更改时有效地更新大量装饰中的位置。
>
>   装饰通过 [facet](https://codemirror.net/docs/ref/#view.EditorView^decorations)提供给编辑器视图。
>   有两种方法可以提供它们
>
>   -   直接提供
>
>   -   间接提供（Indirect decorations）
>       
>       通过一个函数调用视图实例来生成一组装饰。必须直接提供显着**改变编辑器垂直布局**的装饰。
>       
>       例如通过**替换换行符**或**插入块小部件**，因为**间接装饰**仅在计算视口后检索。
>       
>       **间接装饰**适用于语法高亮或搜索匹配高亮，您可能只想在 [视口](https://codemirror.net/docs/ref/#view.EditorView.viewport)或当前[可见范围](https://codemirror.net/docs/ref/#view.EditorView.visibleRanges)内渲染装饰，这对性能有很大帮助。
>
...
原因：**[visibleRanges](https://codemirror.net/docs/ref/#view.EditorView.visibleRanges)** 相关的类
...
%a [【CodeMirror官网示例】下划线命令](https://codemirror.net/examples/decoration/#underlining-command)

下划线案例

>   假设我们要实现一个编辑器扩展，允许用户在文档的某些部分添加下划线。
>   为此，我们可以定义一个[**状态字段**](https://codemirror.net/docs/ref/#state.StateField)来跟踪文档的哪些部分带有下划线，并提供绘制这些下划线的[标记装饰。](https://codemirror.net/docs/ref/#view.Decoration^mark)
...
如果使用的Obsidian插件，需要将注册命令 `keymap.of` 的方式**改写**一下，例如：
...
因此，我们必须首先定义一个子类 [`WidgetType`](https://codemirror.net/docs/ref/#view.WidgetType)绘制小部件。 
