...
Gan算法，例如大头动画设计，如[mimic](https://illustmimic.com/zh/)
...
入门教程：https://www.bilibili.com/read/cv16525687
...
|            | Stable diffusion           | Disco diffusion                    |
| ---------- | -------------------------- | ---------------------------------- |
| 速度       | 快，数十秒一张             | 慢，十数分钟或以上一张             |
| 费用       | 0.01欧一张，注册送两欧     | 免费，但每日算力有限，估计二三十张 |
| 人 物 表现 | 从人到物到景的拟合情况较好 | 适合风景图，氛围图等，不适合人物图 |
| 图片大小   | 小，默认512*512            | 中，默认1280*768                   |
| 方便成都   | 无需科学上网               | 需要科学上网（谷歌）               |
...
\[mimic](https://illustmimic.com/zh/)
...
露出腋下 , armpits
口爆 , cum_in_mouth
含着套 , condoms_in_mouth
舔精 , cum_on_tongue
颜射 , cum_on_facial
骑乘位 , straddling , girl_on_top
向前趴 , lying
...
- tag，AI越熟练的tag，生成的图像完成的越高，反之则越抽象
- 采样步数，越高的步数图像越精细。注意是精细而不是好看，因为如果处理方向不对图只会越来越抽象，因此步数高是有效果，但不是完全的正相关，而且简单tag时高到一定程度时效果就不明显了
- 采样方法：请看我评论区另发的图片
- CFG规模：通过控制变量对比得知，随着数值的增大，生成画面：1）画面逐渐明亮；2）色彩逐渐鲜艳；3）层次逐渐丰富；4）风格逐渐多样化。但是该数值超过20时，画面出现类似过曝的现象，色彩溢出明显，线条出现问题。经过反复的调整，该数值通常在5-15范围较合适。

3.真实图片转动漫

- 使用img2img生成，打上“illustration”tag，使用采样方式“DPM2 a”
- 最好加上人类的tag，比如“girl”
