...
![](copy1. 循环序列模型.assets/ae2970d80a119cd341ef31c684bfac49.png)
...
![](copy1. 循环序列模型.assets/cccbc03192af67a089b53d7940659505.png)
...
![](copy1. 循环序列模型.assets/a45c8066f935c6f29d00a95e36cb6662.png)
...
![](copy1. 循环序列模型.assets/8deca8a84f06466155d2d8d53d26e05d.png)
...
![](copy1. 循环序列模型.assets/1653ec3b8eb718ca817d3423ae3ca643.png)
...
![](copy1. 循环序列模型.assets/cb041c33b65e17600842ebf87174c4f2.png)
...
![](copy1. 循环序列模型.assets/140529e4d7531babb5ba21778cd88bc3.png)
...
![](copy1. 循环序列模型.assets/19cbb2d356a2a6e0f35aa2a946b23a2a.png)
...
![](copy1. 循环序列模型.assets/27afdd27f45ad8ddf78677af2a3eeaf8.png)

接下来为了简化这些符号，我要将这部分（$W_{\text{aa}}a^{<t -1>} +W_{\text{ax}}x^{<t>}$）（上图编号1所示）以更简单的形式写出来，我把它写做$a^{<t>} =g(W_{a}\left\lbrack a^{< t-1 >},x^{<t>} \right\rbrack +b_{a})$（上图编号2所示），那么左右两边划线部分应该是等价的。所以我们定义$W_{a}$的方式是将矩阵$W_{aa}$和矩阵$W_{ax}$水平并列放置，$[ <span class="md-search-hit">{{</span>W}_{aa}}\vdots <span class="md-search-hit">{{</span>W}_{ax}}]=W_{a}$（上图编号3所示）。举个例子，如果$a$是100维的，然后延续之前的例子，$x$是10,000维的，那么$W_{aa}$就是个$（100，100）$维的矩阵，$W_{ax}$就是个$（100，10,000）$维的矩阵，因此如果将这两个矩阵堆起来，$W_{a}$就会是个$（100，10,100）$维的矩阵。

用这个符号（$\left\lbrack a^{< t - 1 >},x^{< t >}\right\rbrack$）的意思是将这两个向量堆在一起，我会用这个符号表示，即$\begin{bmatrix}a^{< t-1 >} \\ x^{< t >} \\\end{bmatrix}$（上图编号4所示），最终这就是个10,100维的向量。你可以自己检查一下，用这个矩阵乘以这个向量，刚好能够得到原来的量，因为此时，矩阵$[ W_{aa}\vdots W_{ax}]$乘以$\begin{bmatrix} a^{< t - 1 >} \\ x^{< t >} \\ \end{bmatrix}$，刚好等于$W_<span class="md-search-hit">{{</span>aa}}a^{<t-1>} + W_<span class="md-search-hit">{{</span>ax}}x^{<t>}$，刚好等于之前的这个结论（上图编号5所示）。这种记法的好处是我们可以不使用两个参数矩阵$W_<span class="md-search-hit">{{</span>aa}}$和$W_{{ax}}$，而是将其压缩成一个参数矩阵$W_{a}$，所以当我们建立更复杂模型时这就能够简化我们要用到的符号。
...
![nn-](copy1. 循环序列模型.assets/rnn-f.png)
...
![](copy1. 循环序列模型.assets/998c7af4f90cd0de0c88f138b61f0168.png)

在之前你已经见过对于前向传播（上图蓝色箭头所指方向）怎样在神经网络中从左到右地计算这些激活项，直到输出所有地预测结果。而对于反向传播，我想你已经猜到了，反向传播地计算方向（上图红色箭头所指方向）与前向传播基本上是相反的。

![](copy1. 循环序列模型.assets/ad9dd74b6ce9bcea14baa289df530d6b.png)
...
![](copy1. 循环序列模型.assets/71a0ed918704f6d35091d8b6d60793e4.png)
...
![nn_cell_backpro](copy1. 循环序列模型.assets/rnn_cell_backprop.png)
...
![](copy1. 循环序列模型.assets/329b0748f7282efc206ea8de6a709833.png)
...
![](copy1. 循环序列模型.assets/14e1df0a7a8cdd1584b2e92e87e23aa7.png)

为了完整性，还要补充一个“**一对一**”（**one-to-one**）的结构（上图编号3所示），这个可能没有那么重要，这就是一个小型的标准的神经网络，输入$x$然后得到输出$y$，我们这个系列课程的前两个课程已经讨论过这种类型的神经网络了。

![](copy1. 循环序列模型.assets/db580f1dfd6095d672fc62cce74ce5e2.png)
...
![](copy1. 循环序列模型.assets/1daa38085604dd04e91ebc5e609d1179.png)
...
![](copy1. 循环序列模型.assets/fa17a85c0e7d9b14633013f2223c877b.png)
...
![](copy1. 循环序列模型.assets/54dfcac1220e3cf567a2c656383e40ec.png)
...
![](copy1. 循环序列模型.assets/986226c39270a1e14643e8658fe6c374.png)
...
![](copy1. 循环序列模型.assets/794b8461aeb485ee61c368c90738523e.png)
...
![](copy1. 循环序列模型.assets/8b901fc8fcab9e16b1fe26b92f4ec546.png)
...
![](copy1. 循环序列模型.assets/1d31771da8ced333968541fbbf67e6f1.png)
...
![](copy1. 循环序列模型.assets/4c7e0878e85865c8fed558375e14b938.png)
...
![](copy1. 循环序列模型.assets/8fb1c4afe30b7a0ede26522b355068ba.png)
...
![](copy1. 循环序列模型.assets/ac5d647140997ba713c376fb097ea0e2.png)
...
![](copy1. 循环序列模型.assets/5e55fb9f6de649031e7f9a4b249f4fea.png)
...
![](copy1. 循环序列模型.assets/32f3b94e4e4523bc413d9687ec29e801.png)
...
![](copy1. 循环序列模型.assets/cfa628f62f1c57ee6213793a438957a3.png)
...
![](copy1. 循环序列模型.assets/c1df3f793dcb1ec681db6757b4974cee.png)
...
![](copy1. 循环序列模型.assets/daf9e8fa30888b6a1b407ca6ea303984.png)
...
![](copy1. 循环序列模型.assets/523650730db3f0d5c05a7192da02f878.png)
...
![](copy1. 循环序列模型.assets/9456d50c55cf0408a3fb2b6e903d85d6.png)
...
![](copy1. 循环序列模型.assets/94e871edbd87337937ce374e71d56e42.png)
...
![ST](copy1. 循环序列模型.assets/LSTM.png)

![STM_rn](copy1. 循环序列模型.assets/LSTM_rnn.png)
...
![](copy1. 循环序列模型.assets/63be3fd3701604f94b45a08f1d8b460d.png)
...
![](copy1. 循环序列模型.assets/48c787912f7f8daee638dd311583d6cf.png)
...
![](copy1. 循环序列模型.assets/053831ff43d039bd5e734df96d8794cb.png)
...
![](copy1. 循环序列模型.assets/8378c2bfe73e1ac9f85d6aa79b71b5eb.png)
...
![](copy1. 循环序列模型.assets/455863a3c8c2dfaa0e5474bfa2c6824d.png)
