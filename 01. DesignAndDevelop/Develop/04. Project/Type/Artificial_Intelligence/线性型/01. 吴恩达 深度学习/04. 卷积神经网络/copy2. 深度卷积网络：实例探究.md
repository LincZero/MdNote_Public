...
![](copy2. 深度卷积网络：实例探究.assets/7fb4f4ae7f3dcd200fcaacd5a3188d51.png)
...
![](copy2. 深度卷积网络：实例探究.assets/a3931d10222ebaf43490935d391eeceb.png)
...
![](copy2. 深度卷积网络：实例探究.assets/5e59b38c9b2942a407b49da84677dae9.png)
...
![](copy2. 深度卷积网络：实例探究.assets/6a9d80274dc6315d9d8c0c27b81548b4.png)
...
![](copy2. 深度卷积网络：实例探究.assets/92575493ecd20003b0b76ac51de0efbb.png)
...
![](copy2. 深度卷积网络：实例探究.assets/1c258ebaf8c400984a0bc777232fe1d6.png)
...
![](copy2. 深度卷积网络：实例探究.assets/a645bff2623ba6f30f01fbc6e3149484.png)
...
![](copy2. 深度卷积网络：实例探究.assets/781ddc74bcc8da430c99b196d0c4c6d4.png)

假设要识别这个图像，在最开始的两层用64个3×3的过滤器对输入图像进行卷积，输出结果是224×224×64，因为使用了**same**卷积，通道数量也一样。**VGG-16**其实是一个很深的网络，这里我并没有把所有卷积层都画出来。

![](copy2. 深度卷积网络：实例探究.assets/349bbded493496f45cf06e0941e6f049.png)
...
![](copy2. 深度卷积网络：实例探究.assets/4a5b3f4c8b34eb8cdca6c36b46af142d.png)
...
![](copy2. 深度卷积网络：实例探究.assets/0a29aeae65a311c56675ad8f1fec2824.png)
...
![](copy2. 深度卷积网络：实例探究.assets/4486bc7028169ae755a01af9c77204f6.png)
...
![](copy2. 深度卷积网络：实例探究.assets/f0a8471f869d8062ba59598c418da7fb.png)
...
![](copy2. 深度卷积网络：实例探究.assets/48bded15cca17581084e3fe0853673b5.png)

这并不是一个残差网络，而是一个普通网络（**Plain network**），这个术语来自**ResNet**论文。

![](copy2. 深度卷积网络：实例探究.assets/131e538bb527859430280becd65b049b.png)

把它变成**ResNet**的方法是加上所有跳跃连接，正如前一张幻灯片中看到的，每两层增加一个捷径，构成一个残差块。如图所示，5个残差块连接在一起构成一个残差网络。

![](copy2. 深度卷积网络：实例探究.assets/6077958a616425d76284cecb43c2f458.png)
...
![](copy2. 深度卷积网络：实例探究.assets/e1106db40c78c2e384305d6474c40d69.png)
...
![](copy2. 深度卷积网络：实例探究.assets/f60f5ca514d4bad1288fc7cbd666dd99.png)
...
![](copy2. 深度卷积网络：实例探究.assets/2af8c71686dc45ced8d66c011714343f.png)

![img](copy2. 深度卷积网络：实例探究.assets/skip_connection_kiank.png)
...
![](copy2. 深度卷积网络：实例探究.assets/cefcaece17927e14eb488cb52d99aaef.png)
...
![](copy2. 深度卷积网络：实例探究.assets/033b59baaa5632368152f5164d17945a.png)

最后，我们来看看**ResNets**的图片识别。这些图片是我从何凯明等人论文中截取的，这是一个普通网络，我们给它输入一张图片，它有多个卷积层，最后输出了一个**Softmax**。

![](copy2. 深度卷积网络：实例探究.assets/216be29ad1287b177b62fb26909bdaeb.png)
...
![](copy2. 深度卷积网络：实例探究.assets/70062fa97916ab79c7ad37282ba1a5f4.png)
...
![](copy2. 深度卷积网络：实例探究.assets/84c62c1d9bdf0a14f20cbeb02e1cec1a.png)

如果是一张6×6×32的图片，那么使用1×1过滤器进行卷积效果更好。具体来说，1×1卷积所实现的功能是遍历这36个单元格，计算左图中32个数字和过滤器中32个数字的元素积之和，然后应用**ReLU**非线性函数。

![](copy2. 深度卷积网络：实例探究.assets/7522d4cbc42b7db1c5a05bc461106590.png)
...
![](copy2. 深度卷积网络：实例探究.assets/f97e6c31c2b27a2d4ef9610b8f32b335.png)

一般来说，如果过滤器不止一个，而是多个，就好像有多个输入单元，其输入内容为一个切片上所有数字，输出结果是6×6过滤器数量。

![](copy2. 深度卷积网络：实例探究.assets/70eba35d0705dc681c40f09a0926061a.png)
...
![](copy2. 深度卷积网络：实例探究.assets/46698c486da9ae184532d773716c77e9.png)
...
![](copy2. 深度卷积网络：实例探究.assets/49a16fdc10769a86355911f9e324c728.png)
...
![](copy2. 深度卷积网络：实例探究.assets/70f3b26fe86c3ec2507b8bb85be8d30c.png)
...
![](copy2. 深度卷积网络：实例探究.assets/4a9d28e64e503ca97ae54236d9d67c93.png)

如果使用1×1卷积，输出结果会是28×28×\#（某个值），假设输出为28×28×64，并且这里只有一个层。

![](copy2. 深度卷积网络：实例探究.assets/f582106ffc2c9e1cf4b07768c7708020.png)

如果使用3×3的过滤器，那么输出是28×28×128。然后我们把第二个值堆积到第一个值上，为了匹配维度，我们应用**same**卷积，输出维度依然是28×28，和输入维度相同，即高度和宽度相同。

![](copy2. 深度卷积网络：实例探究.assets/61a9e2eacff92380df9ea0149cc25e90.png)

或许你会说，我希望提升网络的表现，用5×5过滤器或许会更好，我们不妨试一下，输出变成28×28×32，我们再次使用**same**卷积，保持维度不变。

![](copy2. 深度卷积网络：实例探究.assets/46cf8f4fe80f47de754d0e0f13945941.png)
...
![](copy2. 深度卷积网络：实例探究.assets/99f8fc7dbe7cd0726f5271aae11b9872.png)
...
![](copy2. 深度卷积网络：实例探究.assets/27894eae037f4fd859d33ebdda1cac9a.png)
...
![](copy2. 深度卷积网络：实例探究.assets/5b3df2904b9d8dc51bd99ccb45ac9f5b.png)
...
![](copy2. 深度卷积网络：实例探究.assets/1fec66d984a3c8c47ff459775d411e71.png)

有时候这被称为瓶颈层，瓶颈通常是某个对象最小的部分，假如你有这样一个玻璃瓶，这是瓶塞位置，瓶颈就是这个瓶子最小的部分。

![](copy2. 深度卷积网络：实例探究.assets/ad873273846297c7cf926ed782e42f9d.png)
...
![](copy2. 深度卷积网络：实例探究.assets/7d160f6eab22e4b9544b28b44da686a6.png)
...
![](copy2. 深度卷积网络：实例探究.assets/75b55231aa2a969b9db92b8282f653f8.png)
...
![](copy2. 深度卷积网络：实例探究.assets/f14bd4e49d0b019d6e59eadfb9bcb829.png)

为了在这个3×3的卷积层中节省运算量，你也可以做相同的操作，这样的话3×3的层将会输出28×28×128。

![](copy2. 深度卷积网络：实例探究.assets/02778a10ddba7091729ee207686b0773.png)

或许你还想将其直接通过一个1×1的卷积层，这时就不必在后面再跟一个1×1的层了，这样的话过程就只有一步，假设这个层的输出是28×28×64。

![](copy2. 深度卷积网络：实例探究.assets/5e7e7894e7ecf646d878df6108b5eb7b.png)

最后是池化层。

![](copy2. 深度卷积网络：实例探究.assets/e0a54ae3bb0a99ce45f6f1d524c100d1.png)
...
![](copy2. 深度卷积网络：实例探究.assets/16a042a0f2d3866909533d409ff2ce3b.png)
...
![](copy2. 深度卷积网络：实例探究.assets/1f2a024a28f664aa704be53cea7ca6f8.png)
...
![](copy2. 深度卷积网络：实例探究.assets/5315d2dbcc3053b0146cabd79304ef1d.png)
...
![](copy2. 深度卷积网络：实例探究.assets/50125f732e2cac3d80d8b5bb33a9026c.png)
...
![](copy2. 深度卷积网络：实例探究.assets/07e7bff2d57987e09f35609928ab2f1e.png)
...
![](copy2. 深度卷积网络：实例探究.assets/ae18beb80a7e47765f97fb51e3a0caf3.png)

我点击这里的按钮（**Clone or download**），将这个**URL**复制到我的剪切板里。

![](copy2. 深度卷积网络：实例探究.assets/aff8fec987aec0390d760e3a956649e4.png)
...
![](copy2. 深度卷积网络：实例探究.assets/1a35107490859ee66487ace54af44fb9.png)
...
![](copy2. 深度卷积网络：实例探究.assets/5bf9647890cd27e98be91a43509e59bf.png)

![](copy2. 深度卷积网络：实例探究.assets/5e50b9408dee3cb785e02fcd31ff6ee5.png)

![](copy2. 深度卷积网络：实例探究.assets/76d2f6cc4c350b44cbbc9e01fb125f69.png)

![](copy2. 深度卷积网络：实例探究.assets/d5759a8b1879ccb1a1caaf35345e885f.png)
...
![](copy2. 深度卷积网络：实例探究.assets/c7e9aa44471aa98de824013e659921c1.png)
...
![](copy2. 深度卷积网络：实例探究.assets/1a0e35cf6dd1892b057554c83f60b1ae.png)
...
![](copy2. 深度卷积网络：实例探究.assets/8f0e69f085991cfc74726983418f6569.png)
...
![](copy2. 深度卷积网络：实例探究.assets/ac520bf9e9facfc026db46b187b513bd.png)
...
![](copy2. 深度卷积网络：实例探究.assets/fbd11f5ad8ff668b8d0ff09882efdeba.png)
...
![](copy2. 深度卷积网络：实例探究.assets/e7079af956d884d6184c4bde62271175.png)
...
![](copy2. 深度卷积网络：实例探究.assets/7cf0e18b739684106548cbbf0c1dd500.png)
...
![](copy2. 深度卷积网络：实例探究.assets/f92337ae2e50a0896d42d45cc7951e43.png)

或许最简单的数据扩充方法就是垂直镜像对称，假如，训练集中有这张图片，然后将其翻转得到右边的图像。对大多数计算机视觉任务，左边的图片是猫，然后镜像对称仍然是猫，如果镜像操作保留了图像中想识别的物体的前提下，这是个很实用的数据扩充技巧。

![](copy2. 深度卷积网络：实例探究.assets/709aa552b6a5f4715620047bacf64753.png)
...
![](copy2. 深度卷积网络：实例探究.assets/e7e2d497b751f798e77e1b040ebbf358.png)
...
![](copy2. 深度卷积网络：实例探究.assets/a5bcde6f0d2c2326be700c0ca441c934.png)
...
![](copy2. 深度卷积网络：实例探究.assets/d69cfc9648f3a37eede074bd28c74c0d.png)
...
![](copy2. 深度卷积网络：实例探究.assets/4ad2dc170b17e7a16be8177453382e40.png)

你可能有存储好的数据，你的训练数据存在硬盘上，然后使用符号，这个圆桶来表示你的硬盘。如果你有一个小的训练数据，你可以做任何事情，这些数据集就够了。

![](copy2. 深度卷积网络：实例探究.assets/a07a48c4a98c820824e85f6c364a0024.png)
...
![](copy2. 深度卷积网络：实例探究.assets/2d9b9ca15ce25598d229470494d796ee.png)

与此同时，**CPU**线程持续加载数据，然后实现任意失真变形，从而构成批数据或者最小批数据，这些数据持续的传输给其他线程或者其他的进程，然后开始训练，可以在**CPU**或者**GPU**上实现训一个大型网络的训练。

![](copy2. 深度卷积网络：实例探究.assets/5ee17d350497cb8cf52881f14cb0d9e8.png)
...
![](copy2. 深度卷积网络：实例探究.assets/c34d9a9d4ee4ae6a180783d3a135765c.png)
...
![](copy2. 深度卷积网络：实例探究.assets/e6701cf4129576648941bfd593a13c77.png)
...
![](copy2. 深度卷积网络：实例探究.assets/c87f6cc9ec9c45ad57a049b6baf0b86d.png)
...
![](copy2. 深度卷积网络：实例探究.assets/7e51335f705120b35fa4ed5444ec5cda.png)
...
![](copy2. 深度卷积网络：实例探究.assets/0d0a3e182ddb9e995af3c6a68c7a72eb.png)
...
![](copy2. 深度卷积网络：实例探究.assets/6027faa79b81f9940281ea36ca901504.png)
...
![](copy2. 深度卷积网络：实例探究.assets/fbb8d5acae8a02c366cea92000577d62.png)
...
![](copy2. 深度卷积网络：实例探究.assets/824971b5b093abce4d152560fead1a8a.png)
