...
![](copy2. 优化算法.assets/ef8c62ba3c82cb37e6ed4783e7717a8d.png)
...
![](copy2. 优化算法.assets/31ee4f6d35c7b509b65f6714574a08f6.png)

你共有5000个**mini-batch**，所以最后得到是$X^{\left\{ 5000 \right\}}$

![](copy2. 优化算法.assets/112c45cf393d896833ffce29e14fe8bc.png)

对$Y$也要进行相同处理，你也要相应地拆分$Y$的训练集，所以这是$Y^{\{1\}}$，然后从$y^{(1001)}$到$y^{(2000)}$，这个叫$Y^{\{2\}}$，一直到$Y^{\{ 5000\}}$。

![](copy2. 优化算法.assets/609ea5be2140af929985951f2aab542c.png)

**mini-batch**的数量$t$组成了$X^{\{ t\}}$和$Y^{\{t\}}$，这就是1000个训练样本，包含相应的输入输出对。

![](copy2. 优化算法.assets/98fd6e4c5eec8866bbf722c489cd6177.png)
...
![](copy2. 优化算法.assets/539405047b6986e05f2a3eb06dd7027d.png)
...
![](copy2. 优化算法.assets/0818dc0f8b7b8c1703d0c596f6027728.png)
...
![](copy2. 优化算法.assets/b5c07d7dec7e54bed73cdcd43e79452d.png)
...
![](copy2. 优化算法.assets/2181fbdc47d4d28840c3da00295548ea.png)
...
![](copy2. 优化算法.assets/6ffa1351889a85cb5247fc7c43555959.png)
...
![](copy2. 优化算法.assets/f5fce700d57a539bebebdc0e5195c5fb.png)
...
![](copy2. 优化算法.assets/bb2398f985f57c5d422df3c71437e5ea.png)
...
![](copy2. 优化算法.assets/0f9d1f236d2a4847e9d0c6eb4c5400dd.png)
...
![](copy2. 优化算法.assets/66128cd2cc8d698c2a1fa351f2a22fb9.png)

你用数据作图，可以得到以下结果，起始日在1月份，这里是夏季初，这里是年末，相当于12月末。

![](copy2. 优化算法.assets/d49c52a460376e00c8e5b40baed2cf23.png)

这里是1月1号，年中接近夏季的时候，随后就是年末的数据，看起来有些杂乱，如果要计算趋势的话，也就是温度的局部平均值，或者说移动平均值。

![](copy2. 优化算法.assets/625746a61a4573b7f8594a3c071044c5.png)
...
![](copy2. 优化算法.assets/4324e926e3ba304e339cda820f61fc28.png)
...
![](copy2. 优化算法.assets/c8d108c5e38cfa4defe77cb517a8c7c2.png)
...
![](copy2. 优化算法.assets/a3b26bbce9cd3d0decba5aa8b26af035.png)
...
![](copy2. 优化算法.assets/81710bf53763e939fdac8bd8736960e6.png)

作图运行后得到黄线。

![](copy2. 优化算法.assets/369ae78c3b63e5b537cc0e30f60eb471.png)
...
![](copy2. 优化算法.assets/6f5438fde578ef4285059d85976d52ed.png)
...
![](copy2. 优化算法.assets/9ab7565d5a3e13a9a525ec6d2f119a79.png)
...
![](copy2. 优化算法.assets/a3f97d59db202127014297ceccf1aacb.png)
...
![](copy2. 优化算法.assets/469d894aae98925841134e9766557fc7.png)

然后我们构建一个指数衰减函数，从0.1开始，到$0.1 \times 0.9$，到$0.1 \times {(0.9)}^{2}$，以此类推，所以就有了这个指数衰减函数。

![](copy2. 优化算法.assets/a0bbcc060cac02d8d3f0b1403484f017.png)

计算$v_{100}$是通过，把两个函数对应的元素，然后求和，用这个数值100号数据值乘以0.1，99号数据值乘以0.1乘以${(0.9)}^{2}$，这是第二项，以此类推，所以选取的是每日温度，将其与指数衰减函数相乘，然后求和，就得到了$v_{100}$。

![](copy2. 优化算法.assets/880dbafaccbd93a4ab0a92a9b8607956.png)

结果是，稍后我们详细讲解，不过所有的这些系数（$0.1,~0.1 \times 0.9,~0.1 \times {(0.9)}^{2},~0.1 \times {(0.9)}^{3}\ldots$），相加起来为1或者逼近1，我们称之为偏差修正，下个视频会涉及。
...
![](copy2. 优化算法.assets/9cef2303cf6d7893020647bfde69b22c.png)

又因此当$\beta=0.9$的时候，我们说仿佛你在计算一个指数加权平均数，只关注了过去10天的温度，因为10天后，权重下降到不到当日权重的三分之一。

![](copy2. 优化算法.assets/7820d206a4afa0a694333e74ad74233b.png)
...
![](copy2. 优化算法.assets/358269d36d06e3c70da3e87d8dc523e4.png)
...
![](copy2. 优化算法.assets/53d041ed225ba1d37f239b6498bd134e.png)
...
![](copy2. 优化算法.assets/26a3c3022a7f7ae7ba0cd27fc74cbcf6.png)
...
![](copy2. 优化算法.assets/f602c9d517a7f6c01fe18171dade17e6.png)
...
![](copy2. 优化算法.assets/54322ca2d7b5aed9739fd079fb3b2b6d.png)
...
![](copy2. 优化算法.assets/f378695f4b475da6546b6ab239d27a3d.png)

慢慢摆动到最小值，这种上下波动减慢了梯度下降法的速度，你就无法使用更大的学习率，如果你要用较大的学习率（紫色箭头），结果可能会偏离函数的范围，为了避免摆动过大，你要用一个较小的学习率。

![](copy2. 优化算法.assets/cc2d415b8ccda9fdaba12c575d4d3c4b.png)
...
![](copy2. 优化算法.assets/be943a4d64800e989a375e7972fefbfe.png)
...
![](copy2. 优化算法.assets/a3af0fb5e49e16c108e72d015ef0fdb6.png)
...
![](copy2. 优化算法.assets/89d51f9b5882cf226111b2c0eda2f3e3.png)
...
![](copy2. 优化算法.assets/01dcc876f49fece3c6ad5f5a158020f6.png)
...
![](copy2. 优化算法.assets/553ee26f6efd82d9996dec5f77e3f12e.png)
...
![](copy2. 优化算法.assets/d43cf7898bd88adff4aaac607c1bd5a1.png)
...
![](copy2. 优化算法.assets/720d168e6679fd689fb87c6c5bcb27fa.png)
...
![](copy2. 优化算法.assets/9ca9bfc160d53b23ea0d1164e6accffe.png)
...
![](copy2. 优化算法.assets/e9858303cd62eacc21759b16a121ff58.png)
...
![](copy2. 优化算法.assets/463d0a973dda9b0c8c04f46100b5e0ad.png)
...
![](copy2. 优化算法.assets/c1b5ed3bb9f4ef32fe47056a017e51bf.png)
...
![](copy2. 优化算法.assets/095feaa609b0029d6abc5c74ef7b3b35.png)
...
![](copy2. 优化算法.assets/7c4141aeec2ef4afc6aa534a486a34c8.png)
...
![](copy2. 优化算法.assets/7e0edfb697e8262dc39a040a987c62bd.png)
...
![](copy2. 优化算法.assets/e1b6dc57b8b73ecf5ff400852c4f7086.png)
...
![](copy2. 优化算法.assets/a662f8be4a3a009b68c30a3e6e0683a3.png)
...
![](copy2. 优化算法.assets/1f7df04b804836fbcadcd258c0b55f74.png)

也就是在这个点，这里是$W_{1}$和$W_{2}$，高度即成本函数$J$的值。

![](copy2. 优化算法.assets/a8c3dfdc238762a9f0edf26e6037ee09.png)
...
![](copy2. 优化算法.assets/ea0d64953a3a7b41ef2997c8fee2f930.png)

就像下面的这种：

![](copy2. 优化算法.assets/c5e480c51363d55e8d5e43df1eee679b.png)
...
![](copy2. 优化算法.assets/011e9625870797d6c0695658b92f606e.png)
...
![](copy2. 优化算法.assets/51d79b3734ccf1abdb0b9795a80d8bb7.png)

我们可以沿着这段长坡走，直到这里，然后走出平稳段。

![](copy2. 优化算法.assets/607bd30801c87ed74bb95c49f218f632.png)
