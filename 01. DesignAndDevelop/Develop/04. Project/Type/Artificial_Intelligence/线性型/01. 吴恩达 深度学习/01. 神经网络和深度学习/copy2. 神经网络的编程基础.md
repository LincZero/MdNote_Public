...
![](copy2. 神经网络的编程基础.assets/269118812ea785aee00f6ffc11b5c882.png)
...
![](copy2. 神经网络的编程基础.assets/1e664a86fa2014d5212bcb88f1c419cf.png)
...
![](copy2. 神经网络的编程基础.assets/e173fd42de5f1953deb617623d5087e8.png)
...
![](copy2. 神经网络的编程基础.assets/12f602ed40ba90540112ae0fee77fadf.png)
...
![](copy2. 神经网络的编程基础.assets/1661e545ce5fd2c27b15444d5b69ec78.png)
...
![](copy2. 神经网络的编程基础.assets/55345ba411053da11ff843bbb3406369.png)
...
![](copy2. 神经网络的编程基础.assets/dfb5731c30b81eced917450d31e860a3.png)
...
![](copy2. 神经网络的编程基础.assets/7e304debcca5945a3443d56bcbdd2964.png)
...
![](copy2. 神经网络的编程基础.assets/f5049dc7ce815b495fbbdf71f23fc66c.png)
...
![1](copy2. 神经网络的编程基础.assets/4c9a27b071ce9162dbbcdad3393061d2.png)
...
![](copy2. 神经网络的编程基础.assets/cbd5ff8c461fcb5a699c4ec4789687b3.jpg)

如图，在第二行给出和之前一样的逻辑回归算法的代价函数（成本函数）

**梯度下降法的形象化说明**

![](copy2. 神经网络的编程基础.assets/a3c81d2c8629d674141def47dc02f312.jpg)
...
![](copy2. 神经网络的编程基础.assets/236774be30d12524a2002c3c484d22d5.jpg)

如图，代价函数（成本函数）$J(w,b)$是一个凸函数(**convex function**)，像一个大碗一样。

![](copy2. 神经网络的编程基础.assets/af11ecd5d72c85f777592f8660678ce6.jpg)

如图，这就与刚才的图有些相反，因为它是非凸的并且有很多不同的局部最小值。由于逻辑回归的代价函数（成本函数）$J(w,b)$特性，我们必须定义代价函数（成本函数）$J(w,b)$为凸函数。
**初始化$w$和$b$，**

![](copy2. 神经网络的编程基础.assets/1b79cca8e1902f0ee24b4eb966755ddd.jpg)

可以用如图那个小红点来初始化参数$w$和$b$，也可以采用随机初始化的方法，对于逻辑回归几乎所有的初始化方法都有效，因为函数是凸函数，无论在哪里初始化，应该达到同一点或大致相同的点。

![](copy2. 神经网络的编程基础.assets/0ad6c298d0ac25ca9b26546bb06d462c.jpg)

我们以如图的小红点的坐标来初始化参数$w$和$b$。

**2. 朝最陡的下坡方向走一步，不断地迭代**

![](copy2. 神经网络的编程基础.assets/bb909b874b2865e66eaf9a5d18cc00e5.jpg)

我们朝最陡的下坡方向走一步，如图，走到了如图中第二个小红点处。

![](copy2. 神经网络的编程基础.assets/c5eda5608fd2f4d846559ed8e89ed33c.jpg)
...
![](copy2. 神经网络的编程基础.assets/5300d40870ec58cb0b8162747b9559b9.jpg)

假定代价函数（成本函数）$J(w)$ 只有一个参数$w$，即用一维曲线代替多维曲线，这样可以更好画出图像。

![](copy2. 神经网络的编程基础.assets/6cdef1989a113fc1caaaaf6ebaaa3549.jpg)

![](copy2. 神经网络的编程基础.assets/60cc674531ac72b2d75b0c447db95e96.jpg)
...
![](copy2. 神经网络的编程基础.assets/27be50001e7a91bd2abaaeaf7aba7cd4.jpg)

对于导数更加形象化的理解就是斜率（**slope**），如图该点的导数就是这个点相切于 $J(w)$的小三角形的高除宽。假设我们以如图点为初始化点，该点处的斜率的符号是正的，即$\frac{dJ(w)}{dw}>0$，所以接下来会向左走一步。

![](copy2. 神经网络的编程基础.assets/4fb3b91114ecb2cd81ec9f3662434d81.jpg)

整个梯度下降法的迭代过程就是不断地向左走，直至逼近最小值点。

![](copy2. 神经网络的编程基础.assets/579fb3957063480420c6a7d294503e97.jpg)

假设我们以如图点为初始化点，该点处的斜率的符号是负的，即$\frac{dJ(w)}{dw}<0$，所以接下来会向右走一步。

![](copy2. 神经网络的编程基础.assets/21541fc771ad8895c18d292dd4734fe7.jpg)
...
![](copy2. 神经网络的编程基础.assets/593eb7e2835b4f3c3aa8185cfa76155c.png)
...
![](copy2. 神经网络的编程基础.assets/efe674e73c693a0c5439b68652ed2ce1.png)
...
![](copy2. 神经网络的编程基础.assets/beee332de6c608239d35a7a4f466594c.png)
...
![](copy2. 神经网络的编程基础.assets/c34ba67cca6cb94c79a2e63cc5749c1f.png)
...
![](copy2. 神经网络的编程基础.assets/5b78b725478a0b4def09539f54c784be.png)
...
可以说，一个神经网络的计算，都是按照前向或反向传播过程组织的。首先我们**计算出一个新的网络的输出（前向过程），紧接着进行一个反向传输操作**。后者我们用来计算出对应的梯度或导数。计算图解释了为什么我们用这种方式组织这些计算过程。在这个视频中，我们将举一个例子说明计算图是什么。让我们举一个比逻辑回归更加简单的，或者说不那么正式的神经网络的例子。

![](copy2. 神经网络的编程基础.assets/5216254e20325aad2dd51975bbc70068.png)
...
![](copy2. 神经网络的编程基础.assets/b1c9294420787ec6d7724d64ed9b4a43.png)
...
![](copy2. 神经网络的编程基础.assets/44c62688d05844b26599653545d24dd4.png)
...
![](copy2. 神经网络的编程基础.assets/d4f37c1db52a999dd68b89564449669f.png)

要解释这个计算过程，其中一种方式是：如果你改变了$a$，那么也会改变$v$，通过改变$v$，也会改变$J$，所以$J$值的净变化量，当你提升这个值（0.001），当你把$a$值提高一点点，这就是$J$的变化量（0.003）。

![](copy2. 神经网络的编程基础.assets/7e9ea7f52cab1a428aa1fb670fbe54e9.png)
...
![](copy2. 神经网络的编程基础.assets/eccab9443ace5d97b50ec283a8f85ba8.png)
...
![](copy2. 神经网络的编程基础.assets/960127d6727a198511fe59459bf4a724.png)
...
![](copy2. 神经网络的编程基础.assets/88f92a16ca8cd36f8252c5c6db9c980b.png)

好，所以在程序里是$dvar$表示导数，你关心的最终变量$J$的导数，有时最后是$L$，对代码中各种中间量的导数，所以代码里这个东西，你用$dv$表示这个值，所以$dv=3$，你的代码表示就是$da=3$。

![](copy2. 神经网络的编程基础.assets/22e98c374e2aaa999f46d27339ce6720.png)
...
![](copy2. 神经网络的编程基础.assets/74a169313e532a2c953ea01b05d57385.png)
...
![](copy2. 神经网络的编程基础.assets/596ea4f3492f8e96ecd560e3899e2700.png)
...
其中$a$是逻辑回归的输出，$y$是样本的标签值。现在让我们画出表示这个计算的**计算图**。
这里先复习下梯度下降法，$w$和$b$的修正量可以表达如下：

$w:=w-a \frac{\partial J(w,b)}{\partial w}$，$b:=b-a\frac{\partial J(w,b)}{\partial b}$

![2017-08-16 16-05-25](copy2. 神经网络的编程基础.assets/03f5f96177ab15d5ead8298ba50300ac.jpg)
...
![2017-08-16 16-07-49](copy2. 神经网络的编程基础.assets/6403f00e5844c3100f4aa9ff043e2319.jpg)
...
![02-10-Gradient [00_02_18][20170816-103833-9]](copy2. 神经网络的编程基础.assets/bf930b1f68d8e0726dda5393afc83672.png)
...
![](copy2. 神经网络的编程基础.assets/8b725e51dcffc53a5def49438b70d925.png)
...
![](copy2. 神经网络的编程基础.assets/e9c7f9f3694453c07fe6b9fe7cf0c4c8.png)
...
c = np.dot(a,b)
toc = time.time()
print("Vectorized version:" + str(1000*(toc-tic)) +"ms") #打印一下向量化的版本的时间，结果1.5027ms
...
toc = time.time()
print(c)
print("For loop:" + str(1000*(toc-tic)) + "ms") #打印for循环的版本的时间，结果474.2951ms
```
...
![](copy2. 神经网络的编程基础.assets/6ed990de06443ed997090b38671ada1a.png)

![](copy2. 神经网络的编程基础.assets/6af2ed5d358b4f4dd50b67c3887867ef.png)

![](copy2. 神经网络的编程基础.assets/77280fc4a2f1d609b58f23cde47b709c.png)
...
让我们看另外一个例子。如果你想计算向量$u=Av$，这时矩阵乘法定义为，矩阵乘法的定义就是：$u_{i} =\sum_{j}^{}{A_{\text{ij}}v_{i}}$，这取决于你怎么定义$u_{i}$值。同样使用非向量化实现，$u=np.zeros(n,1)$， 并且通过两层循环$for(i):for(j):$，得到$u[i]=u[i]+A[i][j]*v[j]$ 。现在就有了$i$ 和 $j$ 的两层循环，这就是非向量化。
向量化方式就可以用$u=np.dot(A,v)$，右边这种向量化实现方式，**消除了两层**循环使得代码运行速度更快。 	

![3](copy2. 神经网络的编程基础.assets/a22eb5a73d2d45b342810eee70a4620c.png)
...
![1](copy2. 神经网络的编程基础.assets/6a7d76b4a66ef5af71d55b8f980a5ab2.png)
...
![4](copy2. 神经网络的编程基础.assets/e28b7cda6504e2a8ff0f0b2f1e258a96.png)

![2](copy2. 神经网络的编程基础.assets/af298e37ade1883eaeb44c822e279d42.png)

希望这个视频给了你一点向量化感觉，减少一层循环使你代码更快，但事实证明我们能做得更好。所以在下个视频，我们将进一步的讲解逻辑回归，你将会看到更好的监督学习结果。在训练中**不需要使用任何 for** 循环，你也可以写出代码去运行整个训练集。到此为止一切都好，让我们看下一个视频。
...
![](copy2. 神经网络的编程基础.assets/3a8a0c9ed33cd6c033103e35c26eeeb7.png)
...
在之前的实现中，我们已经**去掉了一个for**循环，但我们仍有一个遍历训练集的循环，如下所示：
...
![](copy2. 神经网络的编程基础.assets/505663d02e8120e30c3d8405f31a8497.jpg)

我们的目标是**不使用for**循环，而是向量，我们可以这么做：
...
我们的目的是**不使用for**循环，所以我们就通过一次迭代实现一次梯度下降，但如果你希望多次迭代进行梯度下降，那么仍然需要**for**循环，放在最外层。不过我们还是觉得一次迭代就进行一次梯度下降，避免使用任何循环比较舒服一些。
...
![](copy2. 神经网络的编程基础.assets/685f36c96c86fef53c3abc8fe509949c.png)
...
![](copy2. 神经网络的编程基础.assets/56f38d09498335ae1155d2102f9b435d.png)

下面使用如下代码计算每列的和，可以看到输出是每种食物(100g)的卡路里总和。

![](copy2. 神经网络的编程基础.assets/c74557c22e724b1aa84ceeb3e5b6685d.png)
...
![](copy2. 神经网络的编程基础.assets/aa852c608c711a73cb4d834a2956d9ae.png)
...
![](copy2. 神经网络的编程基础.assets/537ee092a262cbdb874fe6e775039823.png)

在numpy中，当一个 $4 \times 1$的列向量与一个常数做加法时，实际上会将常数扩展为一个 $4 \times 1$的列向量，然后两者做逐元素加法。结果就是右边的这个向量。这种广播机制对于行向量和列向量均可以使用。

再看下一个例子。

![](copy2. 神经网络的编程基础.assets/d03358cec4b5ea22eb5dbef06fb6a1b7.png)
...
![](copy2. 神经网络的编程基础.assets/0fd16b22ad9b31d531ad7aa50f95cbbc.png)

这里相当于是一个 $m \times n$ 的矩阵加上一个 $m \times 1$ 的矩阵。在进行运算时，会先将 $m \times 1$ 矩阵水平复制 $n$ 次，变成一个 $m \times n$ 的矩阵，然后再执行逐元素加法。

广播机制的一般原则如下：

![](copy2. 神经网络的编程基础.assets/bdc1b1f6f0ba18659f140e47b26bf38b.png)
...
![](copy2. 神经网络的编程基础.assets/695618c70fd2922182dc89dca8eb83cc.png)
...
![](copy2. 神经网络的编程基础.assets/a44df591ad815cc67d74a275bd444342.png)
...
![](copy2. 神经网络的编程基础.assets/264aac3c4b28d894821771308be61417.png)

如果你输出 $a$ 和 $a$ 的转置的乘积，然后会返回给你一个向量的外积，是吧？所以这两个向量的外积返回给你的是一个矩阵。

![](copy2. 神经网络的编程基础.assets/424925f28c12891f7807b24e5bb41d8b.png)
...
![](copy2. 神经网络的编程基础.assets/5648345cf0ca282be13961bdc5b1681e.png)
...
![](copy2. 神经网络的编程基础.assets/2102548104812bdfbccd421f3848f900.png)
...
![](copy2. 神经网络的编程基础.assets/147d3a15b0a4dae283662b2c44f91778.png)
...
![](copy2. 神经网络的编程基础.assets/777048a0bae0db1cb0af3070a2a36aa4.png)
...
![](copy2. 神经网络的编程基础.assets/e218670e323ffef8c7f182184ae133a3.png)
...
![](copy2. 神经网络的编程基础.assets/97a410e8ab2e386e2f617b00b5c60dfe.png)
...
![](copy2. 神经网络的编程基础.assets/8764cd16b111b5601a5bbb95ed600fe5.png)
...
![](copy2. 神经网络的编程基础.assets/5b658991f4c61d088aa962310cee99eb.png)
...
![](copy2. 神经网络的编程基础.assets/c389bc9f882cf1a7d8ebbcf479b1b534.png)
...
![](copy2. 神经网络的编程基础.assets/09d86925e834ff665b6073fc2129fb57.png)
...
![](copy2. 神经网络的编程基础.assets/5a3c2208dfbc9dbcfac57fde07dfac0e.png)
...
![](copy2. 神经网络的编程基础.assets/6ce9e585fab07fe9fc124dd2dc14bdbf.png)
