...
- [【C语言中文网】Qt元对象和属性系统详解](http://c.biancheng.net/view/1826.html)
...
2. 在一个类的 private 部分声明 Q_OBJECT宏，使得类可以使用元对象的特性，如**动态属性、信号与槽**
3. MOC（元对象编译器）为每个 **QObject 的子类**提供必要的代码来实现元对象系统的特性。
   构建项目时，MOC 工具读取 C++ 源文件，当它发现类的定义里有 Q_OBJECT 宏时，它就会为这个类生成另外一个包含有元对象支持代码的 C++ 源文件，这个生成的源文件连同类的实现文件一起被编译和连接。

### 作用

作用（基本作用）：Qt 的元对象系统（Meta-Object System）提供了
...
作用（函数的具体表现）：除了信号与槽机制外，元对象还提供如下一些功能：
...
这里的跨工程指不同时编译的两个工程，会引发moc相关报错

### 存在问题举例

例如：
插件接口在工程A，插件使用了Q_OBJECT声明启用了元对象系统。
插件实现在工程B，工程B使用工程A的插件接口文件。

则报错：

```c++
error: 
LNK2001: 无法解析的外部符号 "public: virtual struct QMetaObject const * __cdecl PluginInterface::metaObject(void)const " (?metaObject@PluginInterface@@UEBAPEBUQMetaObject@@XZ)
...
...
...
似乎是因为pro文件不知道使用了那个元文件，**没有使用moc工具编译**一次工程A的插件**接口文件**

### 解决方法

两个工程各有一个拷贝

或者 “添加现有文件” 的方式，主要目的是在pro文件中的HEADERS中添加指定文件，加入moc编译列表中
