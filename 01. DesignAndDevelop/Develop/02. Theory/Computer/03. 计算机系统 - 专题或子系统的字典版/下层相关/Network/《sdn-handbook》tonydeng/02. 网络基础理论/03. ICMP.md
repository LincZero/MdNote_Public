...
![ICMP报文类型](03. ICMP.assets/icmp-message-type.png)
...
![ICMP时间戳请求头](03. ICMP.assets/icmp-timestamp-headers.jpg)
...
![ICMP差错报文类型](03. ICMP.assets/icmp-error-message.png)
...
`ping` 程序提供一个记录路由选项，但有局限性（不是所有的路由机都支持这个选项，而且IP首部选项字段最多也只能存储9个`IP`地址）。因此开发`traceroute`是必要的。 
