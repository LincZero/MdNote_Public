...
![dhcp session](07. DHCP和DNS.assets/DHCP_session_en.svg)
...
1. 当查询数据多大以至于产生了数据截断(`TC标志为1`)，这时，需要利用`TCP`的分片能力来进行数据传输（看[`TCP`的相关章节](https://tonydeng.github.io/sdn-handbook/basic/tcp.html)）。
